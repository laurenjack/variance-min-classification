...
Each GPU trains one model with width k, k+8, k+16, ..., k+8*(N-1).

Usage:
    # Train models starting at k=72 (one model per available GPU)
    python -m jl.double_descent.resnet18.resnet18_main --output-path ./output

    # For quick smoke test:
    python -m jl.double_descent.resnet18.resnet18_main --output-path ./output --epochs 10

    # On 8 GPUs with default k-start=72, trains k=72,80,88,96,104,112,120,128
    # On 1 GPU with k-start=72, trains k=72
"""
...
def main():
    ...
    # Compute k values for this run (one k per GPU, incrementing by 8)
    k_values = [config.k_start + 8 * i for i in range(num_gpus)]
