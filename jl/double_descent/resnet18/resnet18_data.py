"""CIFAR-10 data loading with label noise and disjoint split support."""
...
class DisjointSplitCIFAR10(Dataset):
    """CIFAR-10 with disjoint training splits and independent label noise.

    Splits the 50K training data into num_splits disjoint subsets.
    Each split gets independent label noise applied after splitting.
    """
    ...
    def __init__(
        ...
        """Initialize DisjointSplitCIFAR10.

        Args:
            root: Root directory for CIFAR-10 data.
            split_id: Which split to use (0 to num_splits-1).
            num_splits: Total number of disjoint splits.
            noise_prob: Probability of corrupting each label in this split.
            transform: Optional transform to apply to images.
            download: Whether to download if not present.
            seed: Base seed for reproducible splitting and noise.
        """
        self.cifar = torchvision.datasets.CIFAR10(
            root=root,
            train=True,
        ...
        # Shuffle indices with fixed seed, then partition into disjoint splits
        n_total = len(self.cifar)
        rng = np.random.RandomState(seed)
        all_indices = rng.permutation(n_total)

        # Compute split boundaries
        split_size = n_total // num_splits
        start_idx = split_id * split_size
        end_idx = start_idx + split_size
        self.indices = all_indices[start_idx:end_idx]

        # Copy labels for this split
        self.labels = [self.cifar.targets[i] for i in self.indices]

        # Apply label noise independently to this split
        if noise_prob > 0:
            self._apply_noise(seed + split_id + 1000)  # Different seed per split
...
def load_cifar10_split(
    ...
    For variance experiments: splits 50K training data into num_splits
    disjoint subsets, each with independent label noise.

    Args:
        split_id: Which split to use (0 to num_splits-1).
        num_splits: Total number of disjoint splits (e.g., 4 for 12.5K each).
        noise_prob: Probability of corrupting each label (applied per split).
        batch_size: Batch size for data loaders.
    ...
    # Create disjoint split dataset
    train_dataset = DisjointSplitCIFAR10(
        root=data_dir,
        split_id=split_id,
    ...
    # Test set is always the full test set (same across all splits)
    test_dataset = NoisyCIFAR10(
        root=data_dir,
        train=False,
        noise_prob=0.0,
