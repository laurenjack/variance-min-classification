...
class ResNet2BlockModel(nn.Module):
    def __init__(self, d, h):
        """
        Constructs a 2-block ResNet model.
        ...
        self.res_block2 = ResNetBlock(d, 10)  # Fixed hidden size of 10
...
def run_experiment(n, d, hidden_sizes, num_runs, learning_rate, num_epochs):
    ...
    num_runs ResNet2Block models. Computes the output on a fixed validation set
    ...
    for h in hidden_sizes:
        ...
        for run in range(num_runs):
            # Instantiate the 2-block ResNet model
            ...
            print(f"d: {d}, Hidden units (first block): {h}, Run: {run + 1}, Validation output sample: {z_val[:3].view(-1)}")
...
def main(n, d_list):
    # Configuration
    hidden_sizes = list(range(1, 41))  # Hidden size for the first block from 1 to 10
    num_runs = 30
    learning_rate = 0.001
    num_epochs = 300
    ...
    plt.title("Mean Variance vs Hidden Size (First Block) for different d values (2-block ResNet Model)")
    plt.legend()
    plt.grid(True)
    plt.show()
