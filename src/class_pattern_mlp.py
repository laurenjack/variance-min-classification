...
bias = nn.CrossEntropyLoss()
variance = nn.CrossEntropyLoss()
...
for epoch in range(num_epochs):
    ...
    for image, label, train_with, train_without in train_loader:
        ...
        indices_for_batch = torch.arange(len(label), dtype=torch.int64)
        y_hat_train = y_hat[model_indices, indices_for_batch] # Shape (m // 2, batch_size, num_classes)
        label_train = labels[model_indices, indices_for_batch] # Shape (m // 2, batch_size, num_classes)
        loss = 0.0
        for j in range(m // 2):
            loss += bias(y_hat_train[j], label_train[j])
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        # print(loss.item())
        print('Train Acc: {}'.format(eval_train_accuracy(y_hat_train, label_train)))
